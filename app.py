...
if st.session_state['products']:
    ...
    if generate_report_button:
        ...
        df['Name'] = df['Name'].astype(str)  # Ensure Name column is string type
        top_products = df.groupby('Name').sum(numeric_only=True).sort_values(by='Quantity', ascending=False).head(5)
        ...
        <div>
            <table>
                ...
                {''.join([f"<tr><td>{index}</td><td>{row['Quantity']}</td></tr>" for index, row in top_products.iterrows()])}
            </table>
        </div>
